...
%
% Answer: clears workspace of all variables
%
...
%
% Answer: for and while. "for" loops are used when there is a known number
% of desired loops whereas "while" loops are for when the the desired
% number of loops is unknown (or will continue until a given condition
% remains true).
% 
...
%
% Answer: NaN stands for "not a number" and generates, for example, when
% solving ">> 0/0"
%
...
% variable. Add one to the value of the variable. (just pick any day)
...
numDecimal = 3.5
numDecimal = int32(numDecimal)
numInteger = int32(numDecimal)
...
vecRandom5 = randi ( [(-10), 10], 1, 5)
vecSubtract = vecRandom5 - 3
nPos = sum(vecRandom5 > 1)
absValue = abs(vecRandom5) 
maxValue = max (vecRandom5)
...
% Establish variables for trapezoid base 1 (base1), base 2 (base2), and height (height):
base1 = 6
base2 = 4
height = 2
% Calculate trapezoid area (trapArea) using these variables:
